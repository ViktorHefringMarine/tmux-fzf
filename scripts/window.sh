...
if [[ -z "$1" ]]; then
    action=$(printf "switch\nlink\nmove\nswap\nrename\nkill\n[cancel]" | eval "$TMUX_FZF_BIN $WINDOW_FZF_OPTIONS")
else
    action="$1"
fi
...
if [[ "$action" == "link" ]]; then
    ...
    src_win_origin=$(printf "%s\n[cancel]" "$windows" | eval "$TMUX_FZF_BIN $WINDOW_FZF_OPTIONS $TMUX_FZF_PREVIEW_OPTIONS $WINDOW_FZF_PREVIEW_FRAME")
    ...
    if [[ "$action" != "switch" ]]; then
        ...
        windows=$(echo "$windows" | grep "^$current_session") #  | sed -e 's/:')
        windows=$(echo "$windows" | sed "s/$current_session://")
        windows=$(echo "$windows" | sed "s/([[:digit:]]*.*$//")
        # windows=$(echo "$windows" | sed "/^[[:digit:]]:/a\\letsgo")
        # windows=$(echo "$windows" | sed "s/\(^[[:digit:]]:\)/\\1")
        # echo $windows
        target_origin=$(printf "%s\n" "$windows" | eval "$TMUX_FZF_BIN $WINDOW_FZF_OPTIONS $TMUX_FZF_PREVIEW_OPTIONS $WINDOW_FZF_PREVIEW_FRAME")
    fi
    ...
    if [[ "$action" == "kill" ]]; then
        ...
        target_swap_origin=$(printf "%s\n[cancel]" "$windows" | eval "$TMUX_FZF_BIN $TMUX_FZF_OPTIONS $TMUX_FZF_PREVIEW_OPTIONS $WINDOW_FZF_PREVIEW_FRAME")
